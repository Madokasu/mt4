...
extern string Var1="General Options";
extern int ExtDepth=12; //  parameter for ZZ
/*extern*/ int ExtIndicator=0; //  determines the indicator that will look for points to build a pattern, while only 0
extern int ExtPoint=6; //  the number of break points counted, if you make more than 3, then it is possible to build from any break point
extern string ExtComplect="0"; //  This is in order to display several indicators on the chart.
extern bool ExtPitchfork=false; //  construction using forks is used
extern bool ExtPitchforkRevers=false; //  flips a broom, why? ask vadimcha
extern bool ExtBack=true; //  if true, objects will be shown in the background
extern bool ExtSpiral=true; //  if true, the spiral will be displayed on the chart
extern bool ExtFan=true; //  if true, then fibonenik will be displayed on the chart
extern bool ExtRec=true; //  if true, the rectangle will be displayed
extern bool ExtArc=true; //  if true, then fiboarka will be displayed on the chart
extern bool ExtFiboLevel=false; //  if true, then fib-levels will be displayed on the chart
extern bool ExtLeftChannel=false; //  if true, the channel lines will be displayed
extern bool ExtRightChannel=false; //  if true, the channel lines will be displayed
extern bool ExtSave=false; //  if true, the construction will be saved on the chart
extern int  MetodAutoScale=1; //  1 - ideal figures are displayed, i.e. the circle will be a circle, not an ellipse, 2 - the calculation suggested by Vadimcha
extern double ExtScale=0; //  sets the arc scale
//  Below the coordinates of the points can be entered with pens.
extern datetime TimePointX=0; //  time point x in format '1980.07.19 12:30'
extern datetime TimePointA=0; //  point A time in format '1980.07.19 12:30'
extern datetime TimePointB=0; //  point B time '1980.07.19 12:30'
extern string Var2="Golden Spiral";
extern int    radius = 5;
...
extern int ExtSpiralWidth=2; //  sets the width of the fiber lines
extern string Var3="FiboFan";
extern double FiboFanMediana1=0.382;
extern double FiboFanMediana2=1.272;
extern int ExtFanStyle=0; //  sets line style
extern int ExtFanWidth=1; //  sets the width of the fiber lines
extern color ExtFanColor=DeepPink; //  fibovenik color
extern string Var4="FiboArc";
extern int ExtArcStyle=0; //  sets the style of lines fiboarki
extern int ExtArcWidth=1; //  sets the width of the lines fiboarki
extern color ExtArcColor=Red; //  arc color
extern string Var5="Rectangle";
extern int ExtRecStyle=4; //  sets rectangle line style
extern int ExtRecWidth=1; //  sets the width of the rectangle lines
extern color ExtRecColor=Yellow; //  color rectangle
extern string Var6="Channel";
extern double ExtFiboLeftChannel=1.618; //  sets the fibo point for building left lines
extern double ExtFiboRightChannel=1.618; //  sets the fibo point for the construction of the right lines
extern int ExtChannelStyle=0; //  sets the line style of the channel
extern int ExtChannelWidth=1; //  sets the channel line width
extern color ExtChannelColor=Blue; //  channel line color
extern string Var7="Pitchfork";
extern int ExtPitchforkStyle=0; //  sets the forks line style
extern int ExtPitchforkWidth=1; //  sets the width of the forks
extern color ExtPitchforkColor=Lime; //  color fork
extern string Var8="Fibo Level";
extern int ExtFiboLevelStyle=0; //  sets the fiber level lines
extern int ExtFiboLevelWidth=1; //  sets the width of the fiber lines
extern color ExtFiboLevelColor=Red; //  fiber level
extern string Var9="Used Fibs";
...
//  Advice - set up for yourself the color, style, width of lines of objects, Fibo levels and remove extern before the corresponding parameters.
...
int init()
  {
   ...
   if(hwnd>0)
   {
     ...
     GPixels=rect[2]; // here the function returns the number of pixels horizontally
     VPixels=rect[3]; // here the function returns the number of pixels vertically
   }
   if(ExtScale==0)
   {
...
int start()
  {
   ...
   switch (ExtIndicator)
     {
      ...
      //  here you can add functions for the calculation of pattern points
    ...
    if(ExtPitchfork==false)
    {
     ...
     if(ExtScale==0)
     {
      ...
      if(MetodAutoScale==2)
      {//  the calculation of this autoscale offered vadimcha to him all the questions
    ...
    if(ExtPitchfork)
   {
    ...
    if((p1<p2 && p1<p4) || (p1>p2 && p1>p4))
     {
      //  in this case, the levels will be less than 100%

       p6=(p5-p3)/FiboFanMediana1+p3;//-tang;
       if(ObjectGetShiftByValue("Mediana"+"_"+ExtDepth+"_"+ExtComplect+"_"+save,p6)<-100)
        {
         Alert("The point to build a fan is too far, try using a larger value for FiboFanMediana1");
        }
     ...
     if((p1<p2 && p1>p4) || (p1>p2 && p1<p4))
     {
      //  in this case, the levels over 100% will match.

       p6=(p5-p3)/FiboFanMediana2+p3;//-tang;
       if(ObjectGetShiftByValue("Mediana"+"_"+ExtDepth+"_"+ExtComplect+"_"+save,p6)<-100)
        {
         Alert("The point for building a fan is too far, try using a lower value for FiboFanMediana2");
        }
...
/*------------------------------------------------------------------+
|  ZigZag_Talex, looking for breaking points on a chart. Amount of points   |
|  set by external extpoint parameter.                            |
+------------------------------------------------------------------*/
void ZZTalex()
{
// variables
...
/*-------------------------------------------------------------------+
/ The function to search the first bar (if it is external) which extremum we will use as a vertex.
/-------------------------------------------------------------------*/
double funk1(int zzbarlow, int ExtDepth)
{
 ...
 if(fbarlow>fbarhigh) {
   // if((Low[zzbarlow]<Low[fbarhigh]) && (High[zzbarlow]<High[fbarhigh]))
   pr=High[zzbarlow];
 }
 if(fbarlow<fbarhigh) {
   // if((Low[zzbarlow]>Low[fbarlow]) && (High[zzbarlow]>High[fbarlow]))
   pr=Low[zzbarlow];
 }
 if(fbarlow==fbarhigh)
 {
...
/*---------------------------------------------------------------+
|  The function of the correct display of the timeframe.            |
+---------------------------------------------------------------*/
string TimeFrame()
{
 switch(Period())
...
/*---------------------------------------------------------------+
|  The function to correctly display the timeframe. End.             |
+---------------------------------------------------------------*/

/*---------------------------------------------------------------+
|   Function to draw a golden spiral.                       |
+---------------------------------------------------------------*/
void GoldenSpiral(datetime t2,double p2,datetime t4,double p4,string save)
{
//  Validation of input data and, if required, their correction
 if(radius <= 0)
     {
       radius = 5;
...
// for golden spiral: cot Alpha = 2/pi * ln(phi)
   ...
   double startAngle; // angle in radians(in radians)
