...
//+---------------------------------------------------------------------+
#property link      "https://madokasu-fx.blogspot.com" // v2.3
//----
...
extern bool UseLog=false;      // true = print log
...
int qPoint=0;              // variable for price normalization
...
int init()
  {
   ...
   int code=161; string Rem="Madokasu_Demark";
...
int start()
  {
   ...
   if(qTime==Time[0]) return(0); qTime=Time[0]; // current starter on 1m tick
                                                // filled and displayed demarcation points
   ...
   Comm=Comm+"---- Madokasu_Demark ----";
   if(Comments==true) {
      Comment(Comm);// TODO: PrintCommentに渡して、他のインジケータコメントと競合しないようにする
      Print(Comm);
   }
   if(UseLog==true) {
      Print(Comm);// とりあえず、ログに出力するようにする
   }
   return(0);
  }
//+------------------------------------------------------------------+
//| 機能しない...
//+------------------------------------------------------------------+
void PrintComment(string Comm)
  {
   string ChartInfo="Madokasu_Label";
   ObjectCreate(ChartInfo,OBJ_LABEL,0,0,0);
   ObjectSetText(ChartInfo,Comm,9,"Arial",Gold);
   ObjectSet(ChartInfo,OBJPROP_CORNER,0);
   ObjectSet(ChartInfo,OBJPROP_XDISTANCE,8);
   ObjectSet(ChartInfo,OBJPROP_YDISTANCE,13);
   return;
  }
//+------------------------------------------------------------------+
//|                                                                  |
   ...
   if(Trend<=0)
     {
      ...
      qExt=Lowest(NULL,0,MODE_LOW,H2-H1-1,H1+1); // local minimum between points
      ...
      if(H1<0 || H2<0) Comm=Comm+"not enough points on the chart for construction\n";
      else
        {
         ...
         Comm=Comm+"\n";
         // Analysis - was the breakdown of the trend line
               ...
               if(qp!="")
                 {
                  ...
                  while(i>0)
                    { // the breakdown is canceled if after the breakdown there was a new low or a close below
                     i--;
                 ...
                 { Text=Text+"False "+DoubleToStr(tmp,qPoint)+"; "; ObjectDelete(Rem);}
              }
            i--;
           }
         ...
         Comm=Comm+Text+"\n";
         // end analysis
         Rem="HL_"+Step; // sobsno trend line
         if(TrendLine)
           {
         ...
         Rem="HHL_"+Step; // trend line breakdown level
         ...
         Rem="HTL_"+Step; // goal lines
         if(TakeLines)
           {
            HT[3]=Low[qExt]+(High[H1]-Low[qExt])*1.618-qLevel; //  additional level
   ...
   if(Trend>=0)
     {
      ...
      qExt=Highest(NULL,0,MODE_HIGH,L2-L1-1,L1+1); // for local minimum between points
      ...
      if(L1<0 || L2<0) Comm=Comm+"not enough points on the chart for construction\n";
      else
        {
         ...
         Comm=Comm+"\n";
         // Analysis - was the trend line broken?
               ...
               if(qp!="")
                 {
                  ...
                  while(i>0)
                    { // breakout is canceled if after breakdown there was a new hi or closing higher
                     i--;
         ...
         Comm=Comm+Text+"\n";
         // end analysis
         Rem="LL_"+Step; // sobsno trend line
         if(TrendLine==1)
           {
            ObjectCreate(Rem,OBJ_TREND,0,0,0,0,0);
         ...
         Rem="HLL_"+Step; // trend line breakdown level
         if(HorizontLine && (Step+BackStep)==1)
           {
            ObjectCreate(Rem,OBJ_HLINE,0,0,0,0,0);
            ObjectSet(Rem,OBJPROP_PRICE1,qLevel);
         ...
         if(TakeLines)
           { // goal lines
            LT[3]=qLevel-High[qExt]+(High[qExt]-Low[L1])*1.618; // additional level
...
int GetNextLowTD(int P)
  {
   int i=P+1;
   while(Buf2[i]>=Low[P] || Buf2[i]==0)
     {
      i++;if(i>showBars-2)return(-1);
     }
   return(i);
  }
// recursive condition check Demark (hi), returns value or 0
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
// recursive condition check Demark (low), returns value or 0
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
string UpHitch(int P,double qLevel)
  { // breakthrough qualifier definition
   string Comm="";
...
string DownHitch(int P,double qLevel)
  { // breakthrough qualifier determination
   string Comm="";
   if(Close[P+1]>Close[P+2]) Comm=Comm+" 1";
   if(P>=0 && Open[P]<qLevel) Comm=Comm+" 2";
